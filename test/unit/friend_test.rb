...
class FriendTest < ActiveSupport::TestCase
  ...
  should "have friends" do
    ...
    assert !profiles(:user).friends.any?{|f| f.id == profiles(:user4).id}
    assert profiles(:user).followings.any?{|f| f.id == profiles(:user4).id} # we need to use activate users only.
    assert profiles(:user2).friends.any?{|f| f.id == profiles(:user).id}
    assert profiles(:user4).friends.any?{|f| f.id == profiles(:user).id}
    assert profiles(:user4).followers.any?{|f| f.id == profiles(:user).id}
  end
