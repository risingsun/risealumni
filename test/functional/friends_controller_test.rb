...
class FriendsControllerTest < ActionController::TestCase
  ...
  should "make a friendship" do
    ...
    post :create, {:profile_id=>profiles(:user2).id, :id=>profiles(:user).id, :format=>'js'}, {:user=>profiles(:user2).id}
    assert_response :success
...
should "not make friend(when invited person is inactive) " do
    post :create, {:profile_id => profiles(:user).id, :id => profiles(:inactive).id, :format=>'js'}, {:user=>profiles(:user).id}
    assert_response :success
